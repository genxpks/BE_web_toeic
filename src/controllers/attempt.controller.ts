...
export async function startAttempt(req: Request, res: Response) {
  ...
  if (mockTest.status !== TestStatus.PUBLISHED) throw new Error('Mock test is not published');
...
export async function saveAnswer(req: Request<AttemptParams>, res: Response) {
  ...
  const questionIds = new Set(
    attempt.mockTest.sections.flatMap((section) => section.parts.flatMap((part) => part.questions.map((question) => question.id))),
  );
  ...
  const nextRemainingTime =
    typeof body.remainingTimeSec === 'number' ? Math.min(attempt.remainingTimeSec, body.remainingTimeSec) : undefined;
...
export async function getAttemptSnapshot(req: Request<AttemptParams>, res: Response) {
  const { attemptId } = req.params;
  const attempt = await prisma.attempt.findUnique({
    where: { id: attemptId },
    include: {
...
export async function getInProgressAttempt(req: Request, res: Response) {
...
export async function submitAttempt(req: Request<AttemptParams>, res: Response) {
  const { attemptId } = req.params;
